...
class _HttpDetachedIncoming extends Stream<Uint8List> {
  ...
  StreamSubscription<Uint8List> listen(void onData(Uint8List event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    ...
    if (subscription != null) {
      ...
      return new Stream<Uint8List>.fromIterable([bufferedData!]).listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
    }
  }
}
 ...
 * upgraded to.
 */
class _HttpParser extends Stream<MockHttpIncoming> {
  // State.
  ...
  MockHttpIncoming? _incoming;
  StreamSubscription<Uint8List>? _socketSubscription;
  bool _paused = true;
  bool _bodyPaused = false;
  final StreamController<MockHttpIncoming> _controller;
  ...
  _HttpParser._(this._requestParser) : _controller = new StreamController<MockHttpIncoming>(sync: true) {
    _controller
      ..onListen = () {
        _paused = false;
      }
  ...
  StreamSubscription<MockHttpIncoming> listen(void onData(MockHttpIncoming event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    ...
    while (_buffer != null && _index < _buffer!.length && _state != _State.FAILURE && _state != _State.UPGRADED) {
      ...
      switch (_state) {
        ...
        case _State.HEADER_START:
          _headers = new MockHttpHeaders(version!);
          if (byte == _CharCode.CR) {
        ...
        case _State.HEADER_VALUE_FOLD_OR_END:
          if (byte == _CharCode.SP || byte == _CharCode.HT) {
            ...
            if (headerField == HttpHeaders.connectionHeader) {
              ...
              final bool isUpgradeCode = (_statusCode == HttpStatus.upgradeRequired) || (_statusCode == HttpStatus.switchingProtocols);
        ...
        case _State.BODY:
          ...
          Uint8List data = new Uint8List.view(buffer.buffer, buffer.offsetInBytes + _index, dataAvailable);
  ...
  void _onDone() {
    ...
    if (_incoming != null) {
      if (_state != _State.UPGRADED && !(_state == _State.START && !_requestParser) && !(_state == _State.BODY && !_chunked && _transferLength == -1)) {
  ...
  MockHttpIncoming _createIncoming(int transferLength) {
    assert(_incoming == null);
    assert(_bodyController == null);
    assert(!_bodyPaused);
    var controller = _bodyController = new StreamController<Uint8List>(sync: true);
    var incoming = _incoming = new MockHttpIncoming(_headers!, transferLength, controller.stream);
    controller
      ..onListen = () {
