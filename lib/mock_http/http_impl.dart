...
abstract class HttpProfiler {
  ...
  static Map<int, MockHttpProfileData> _profile = {};

  static MockHttpProfileData startRequest(
    String method,
    Uri uri, {
    MockHttpProfileData? parentRequest,
  }) {
    final data = MockHttpProfileData(method, uri, parentRequest?._timeline);
    _profile[data.id] = data;
    return data;
  }

  static MockHttpProfileData? getHttpProfileRequest(int id) => _profile[id];
...
class MockHttpProfileEvent {
  MockHttpProfileEvent(this.name, this.arguments);
...
class MockHttpProfileData {
  MockHttpProfileData(String method, this.uri, TimelineTask? parent)
  ...
  void requestEvent(String name, {Map? arguments}) {
    ...
    requestEvents.add(MockHttpProfileEvent(name, arguments));
    _updated();
  }
  ...
  String? requestError;
  final requestEvents = <MockHttpProfileEvent>[];
...
abstract class MockServiceObject {
  int __serviceId = 0;
...
class MockCopyingBytesBuilder implements BytesBuilder {
  ...
  MockCopyingBytesBuilder([int initialCapacity = 0]) : _buffer = (initialCapacity <= 0) ? _emptyList : new Uint8List(_pow2roundup(initialCapacity));
  ...
  Uint8List takeBytes() {
    ...
    var buffer = new Uint8List.view(_buffer.buffer, _buffer.offsetInBytes, _length);
    clear();
    return buffer;
  }
  ...
  Uint8List toBytes() {
    if (_length == 0) return _emptyList;
    return new Uint8List.fromList(new Uint8List.view(_buffer.buffer, _buffer.offsetInBytes, _length));
  }
...
class MockHttpIncoming extends Stream<Uint8List> {
  ...
  MockHttpIncoming(this.headers, this._transferLength, this._stream);

  StreamSubscription<Uint8List> listen(void onData(Uint8List event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    hasSubscriber = true;
...
abstract class MockHttpInboundMessageListInt extends Stream<List<int>> {
  final MockHttpIncoming _incoming;
  List<Cookie>? _cookies;

  MockHttpInboundMessageListInt(this._incoming);

  List<Cookie> get cookies => _cookies ??= headers._parseCookies();

  MockHttpHeaders get headers => _incoming.headers;
...
abstract class MockHttpInboundMessage extends Stream<Uint8List> {
  final MockHttpIncoming _incoming;
  List<Cookie>? _cookies;

  MockHttpInboundMessage(this._incoming);
...
class MockHttpRequest extends MockHttpInboundMessage implements HttpRequest {
  final HttpResponse response;

  final MockHttpServer _httpServer;

  final MockHttpConnection _httpConnection;

  _HttpSession? _session;

  Uri? _requestedUri;

  MockHttpRequest(this.response, MockHttpIncoming _incoming, this._httpServer, this._httpConnection) : super(_incoming) {
    ...
    if (_httpServer._sessionManagerInstance != null) {
      ...
      var sessionIds = cookies.where((cookie) => cookie.name.toUpperCase() == _DART_SESSION_ID).map<String>((cookie) => cookie.value);
...
class MockHttpClientResponse extends MockHttpInboundMessageListInt implements HttpClientResponse {
  ...
  final MockHttpClientRequest _httpRequest;

  // The compression state of this response.
  final HttpClientResponseCompressionState compressionState;

  final MockHttpProfileData? _profileData;

  MockHttpClientResponse(MockHttpIncoming _incoming, this._httpRequest, this._httpClient, this._profileData)
  ...
  static HttpClientResponseCompressionState _getCompressionState(MockHttpClient httpClient, MockHttpHeaders headers) {
    if (headers.value(HttpHeaders.contentEncodingHeader) == "gzip") {
      return httpClient.autoUncompress ? HttpClientResponseCompressionState.decompressed : HttpClientResponseCompressionState.compressed;
    } else {
      return HttpClientResponseCompressionState.notCompressed;
    }
  }

  int get statusCode => _incoming.statusCode ?? HttpStatus.ok;
    ...
    if (followLoops != true) {
      for (var redirect in redirects) {
        if (redirect.location == url) {
          return new Future.error(new RedirectException("Redirect loop detected", redirects));
        }
      }
    }
  ...
  bool get _shouldAuthenticateProxy {
    ...
    return statusCode == HttpStatus.proxyAuthenticationRequired && challenge != null && challenge.length == 1;
  }
  ...
  Future<HttpClientResponse> _authenticate(bool proxyAuth) {
    ...
    Future<HttpClientResponse> retry() {
      ...
      return drain().then((_) {
        return _httpClient._openUrlFromRequest(_httpRequest.method, _httpRequest.uri, _httpRequest).then((request) => request.close());
      });
    }

    List<String>? authChallenge() {
      return proxyAuth ? headers[HttpHeaders.proxyAuthenticateHeader] : headers[HttpHeaders.wwwAuthenticateHeader];
    }

    _Credentials? findCredentials(_AuthenticationScheme scheme) {
      return proxyAuth ? _httpClient._findProxyCredentials(_httpRequest._proxy, scheme) : _httpClient._findCredentials(_httpRequest.uri, scheme);
    }
    ...
    Future<bool> requestAuthentication(_AuthenticationScheme scheme, String? realm) {
      if (proxyAuth) {
        ...
        if (!proxy.isDirect) {
          return authenticateProxy(proxy.host!, proxy.port!, scheme.toString(), realm);
        }
      }
    ...
    _HeaderValue header = _HeaderValue.parse(challenge[0], parameterSeparator: ",");
    _AuthenticationScheme scheme = new _AuthenticationScheme.fromString(header.value);
...
class _IOSinkImpl extends _StreamSinkImpl<List<int>> implements IOSink {
  ...
  _IOSinkImpl(StreamConsumer<List<int>> target, this._encoding, this._profileData) : super(target);
...
abstract class MockHttpOutboundMessage<T> extends _IOSinkImpl {
  ...
  final Uri _uri;
  final MockHttpOutgoing _outgoing;

  final MockHttpHeaders headers;

  MockHttpOutboundMessage(Uri uri, String protocolVersion, MockHttpOutgoing outgoing, MockHttpProfileData? profileData, {MockHttpHeaders? initialHeaders})
      : _uri = uri,
        headers = new MockHttpHeaders(protocolVersion,
            defaultPortForScheme: uri.scheme == 'https' ? HttpClient.defaultHttpsPort : HttpClient.defaultHttpPort, initialHeaders: initialHeaders),
        _outgoing = outgoing,
...
class MockHttpResponse extends MockHttpOutboundMessage<HttpResponse> implements HttpResponse {
  int _statusCode = 200;
  String? _reasonPhrase;
  List<Cookie>? _cookies;
  MockHttpRequest? _httpRequest;
  Duration? _deadline;
  Timer? _deadlineTimer;

  MockHttpResponse(Uri uri, String protocolVersion, MockHttpOutgoing outgoing, HttpHeaders defaultHeaders, String? serverHeader)
      : super(uri, protocolVersion, outgoing, null, initialHeaders: defaultHeaders as MockHttpHeaders) {
  ...
  void _writeHeader() {
    BytesBuilder buffer = new MockCopyingBytesBuilder(_OUTGOING_BUFFER_SIZE);
...
class MockHttpClientRequest extends MockHttpOutboundMessage<HttpClientResponse> implements HttpClientRequest {
  final String method;
  final Uri uri;
  ...
  final MockHttpClient _httpClient;
  final MockHttpClientConnection _httpClientConnection;
  final MockHttpProfileData? _profileData;

  final Completer<HttpClientResponse> _responseCompleter = new Completer<HttpClientResponse>();
  ...
  Future<HttpClientResponse> get done => _response ??= Future.wait([_responseCompleter.future, super.done], eagerError: true).then((list) => list[0]);
  ...
  void _onIncoming(MockHttpIncoming incoming) {
    if (_aborted) {
      return;
    }
    final response = MockHttpClientResponse(incoming, this, _httpClient, _profileData);
  ...
  void _writeHeader() {
    ...
    // Write headers.
    headers._build(buffer, skipZeroContentLength: method == "CONNECT" || method == "DELETE" || method == "GET" || method == "HEAD");
    buffer.addByte(_CharCode.CR);
    buffer.addByte(_CharCode.LF);
    Uint8List headerBytes = buffer.takeBytes();
    _outgoing.setHeader(headerBytes, headerBytes.length);
...
// compression.
class MockHttpGZipSink extends ByteConversionSink {
  final _BytesConsumer _consume;

  MockHttpGZipSink(this._consume);
  ...
  void addSlice(List<int> chunk, int start, int end, bool isLast) {
    if (chunk is Uint8List) {
      _consume(new Uint8List.view(chunk.buffer, chunk.offsetInBytes + start, end - start));
    } else {
...
class MockHttpOutgoing implements StreamConsumer<List<int>> {
  static const List<int> _footerAndChunk0Length = const [_CharCode.CR, _CharCode.LF, 0x30, _CharCode.CR, _CharCode.LF, _CharCode.CR, _CharCode.LF];

  static const List<int> _chunk0Length = const [0x30, _CharCode.CR, _CharCode.LF, _CharCode.CR, _CharCode.LF];
  ...
  MockHttpOutboundMessage? outbound;

  MockHttpOutgoing(this.socket);
  ...
  Future<void>? writeHeaders({bool drainRequest = true, bool setOutgoing = true}) {
    ...
    if (response is MockHttpResponse) {
      // Server side.
      if (response._httpRequest!._httpServer.autoCompress && response.bufferOutput && response.headers.chunkedTransferEncoding) {
        ...
        if (acceptEncodings != null &&
            ...
            acceptEncodings.expand((list) => list.split(",")).any((encoding) => encoding.trim().toLowerCase() == "gzip")) {
      ...
      if (chunked) {
        ...
        if (contentLength != null) {
          ...
          if (_bytesWritten > contentLength) {
            controller.addError(new HttpException("Content size exceeds specified contentLength. "
  ...
  Future close() {
    ...
    Future finalize() {
      ...
      if (chunked) {
        if (_gzip) {
          ...
          if (_gzipBufferLength > 0) {
            _gzipSink!.add(new Uint8List.view(_gzipBuffer!.buffer, _gzipBuffer!.offsetInBytes, _gzipBufferLength));
          }
          _gzipBuffer = null;
          _gzipSink!.close();
          _gzipAdd = null;
      ...
      if (_length > 0) {
        socket.add(new Uint8List.view(_buffer!.buffer, _buffer!.offsetInBytes, _length));
      }
  ...
  void set gzip(bool value) {
    ...
    if (value) {
      ...
      _gzipSink = new ZLibEncoder(gzip: true).startChunkedConversion(new MockHttpGZipSink((data) {
  ...
  bool _ignoreError(error) => (error is SocketException || error is TlsException) && outbound is HttpResponse;
  ...
  void _addGZipChunk(List<int> chunk, void add(List<int> data)) {
    ...
    if (chunk.length > gzipBuffer.length - _gzipBufferLength) {
      add(new Uint8List.view(gzipBuffer.buffer, gzipBuffer.offsetInBytes, _gzipBufferLength));
      _gzipBuffer = new Uint8List(_OUTGOING_BUFFER_SIZE);
      _gzipBufferLength = 0;
    }
  ...
  List<int> _chunkHeader(int length) {
    const hexDigits = const [0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46];
    if (length == 0) {
...
class MockHttpClientConnection {
  ...
  Completer<MockHttpIncoming>? _nextResponseCompleter;
  Future<Socket>? _streamFuture;

  MockHttpClientConnection(this.key, this._socket, this._httpClient, [this._proxyTunnel = false, this._context])
    ...
    _subscription = _httpParser.listen((incoming) {
      ...
      if (_nextResponseCompleter == null) {
        throw new HttpException("Unexpected response (unsolicited response without request).", uri: _currentUri);
      }
      ...
      if (incoming.statusCode == 100) {
        incoming.drain().then((_) {
          ...
          _nextResponseCompleter!.completeError(new HttpException(error.message, uri: _currentUri), stackTrace);
      ...
      _nextResponseCompleter?.completeError(new HttpException("Connection closed before response was received", uri: _currentUri));
      _nextResponseCompleter = null;
      close();
    });
  }

  MockHttpClientRequest send(Uri uri, int port, String method, _Proxy proxy, MockHttpProfileData? profileData) {
    if (closed) {
      throw new HttpException("Socket closed before request was sent", uri: uri);
    }
    _currentUri = uri;
    ...
    var outgoing = new MockHttpOutgoing(_socket);

    // Create new request object, wrapping the outgoing connection.
    var request = new MockHttpClientRequest(outgoing, uri, method, proxy, _httpClient, this, profileData);
    ...
    _streamFuture = outgoing.done.then<Socket>((Socket s) {
      ...
      var nextResponseCompleter = new Completer<MockHttpIncoming>();
      ...
      if (sCurlCommands != null && sCurlCommands!.isNotEmpty) {
        curlList = sCurlCommands!;
      }

      final String key = json.encode(curlList);
      String? serverResponse;
      switch (sMockWay) {
        case MOCK_WAY.MOCK_JSON:
          serverResponse = sMockJsonPluginResult[key];
          break;
        case MOCK_WAY.MOCK_HTTP:
          var result = Process.runSync('curl', curlList);
          serverResponse = result.stdout;
          break;
        case MOCK_WAY.MOCK_JSON_HTTP_GENERATE:
          var result = Process.runSync('curl', curlList);
          serverResponse = result.stdout;
          sMockJsonPluginResult[key] = serverResponse;
          break;
      }
      final Stream<Uint8List> _delegate = Stream<Uint8List>.fromIterable([Uint8List.fromList(utf8.encode(serverResponse!))]);
      MockHttpHeaders head = MockHttpServer._initDefaultResponseHeaders();
      request._onIncoming(MockHttpIncoming(head, 1, _delegate));
  ...
  Future<Socket> detachSocket() {
    return _streamFuture!.then((_) => new _DetachedSocket(_socket, _httpParser.detachIncoming()));
  }
  ...
  Future<MockHttpClientConnection> createProxyTunnel(
      String host, int port, _Proxy proxy, bool callback(X509Certificate certificate), MockHttpProfileData? profileData) {
    ...
    MockHttpProfileData? proxyProfileData;
    if (profileData != null) {
    ...
    MockHttpClientRequest request = send(Uri(host: host, port: port), port, method, proxy, proxyProfileData);
    if (proxy.isAuthenticated) {
      // If the proxy configuration contains user information use that
      // for proxy basic authorization.
      String auth = _CryptoUtils.bytesToBase64(utf8.encode("${proxy.username}:${proxy.password}"));
    ...
    return request.close().then((response) {
      ...
      var socket = (response as MockHttpClientResponse)._httpRequest._httpClientConnection._socket;
      return SecureSocket.secure(socket, host: host, context: _context, onBadCertificate: callback);
    }).then((secureSocket) {
      String key = MockHttpClientConnection.makeKey(true, host, port);
      profileData?.requestEvent('Proxy tunnel established');
      return new MockHttpClientConnection(key, secureSocket, request._httpClient, true);
    });
  }

  HttpConnectionInfo? get connectionInfo => MockHttpConnectionInfo.create(_socket);
...
class _ConnectionInfo {
  final MockHttpClientConnection connection;
  final _Proxy proxy;
...
class _ConnectionTarget {
  ...
  final Set<MockHttpClientConnection> _idle = new HashSet();
  final Set<MockHttpClientConnection> _active = new HashSet();
  ...
  MockHttpClientConnection takeIdle() {
    assert(hasIdle);
    MockHttpClientConnection connection = _idle.first;
  ...
  void addNewActive(MockHttpClientConnection connection) {
    _active.add(connection);
  }

  void returnConnection(MockHttpClientConnection connection) {
  ...
  void connectionClosed(MockHttpClientConnection connection) {
  ...
  Future<_ConnectionInfo> connect(String uriHost, int uriPort, _Proxy proxy, MockHttpClient client, MockHttpProfileData? profileData) {
    if (hasIdle) {
    ...
    if (maxConnectionsPerHost != null && _active.length + _connecting >= maxConnectionsPerHost) {
      var completer = new Completer<_ConnectionInfo>();
      _pending.add(() {
        completer.complete(connect(uriHost, uriPort, proxy, client, profileData));
      });
    ...
    var connection = MockHttpClientConnection(key, MockSocket(_delegate), client, false, context);
...
class MockHttpClient implements HttpClient {
  ...
  final Map<String, _ConnectionTarget> _connectionTargets = new HashMap<String, _ConnectionTarget>();
  ...
  Future<HttpClientRequest> open(String method, String host, int port, String path) {
    ...
    Uri uri = new Uri(scheme: "http", host: host, port: port, path: path, query: query);
  ...
  void addCredentials(Uri url, String realm, HttpClientCredentials cr) {
    _credentials.add(new _SiteCredentials(url, realm, cr as MockHttpClientCredentials));
  }
  ...
  void addProxyCredentials(String host, int port, String realm, HttpClientCredentials cr) {
    _proxyCredentials.add(new _ProxyCredentials(host, port, realm, cr as MockHttpClientCredentials));
  }
  ...
  Future<MockHttpClientRequest> _openUrl(String method, Uri uri) {
    if (_closing) {
    ...
    if (method != "CONNECT") {
      if (uri.host.isEmpty) {
        ...
        throw new ArgumentError("Unsupported scheme '${uri.scheme}' in URI $uri");
      }
    }
    ...
    MockHttpProfileData? profileData;
    if (HttpClient.enableTimelineLogging) {
      profileData = HttpProfiler.startRequest(method, uri);
    }
    return _getConnection(uri.host, port, proxyConf, isSecure, profileData).then((_ConnectionInfo info) {
      MockHttpClientRequest send(_ConnectionInfo info) {
      ...
      if (info.connection.closed) {
        return _getConnection(uri.host, port, proxyConf, isSecure, profileData).then(send);
      }
      return send(info);
    }, onError: (error) {
  ...
  Future<MockHttpClientRequest> _openUrlFromRequest(String method, Uri uri, MockHttpClientRequest previous) {
    // If the new URI is relative (to either '/' or some sub-path),
    // construct a full URI from the previous one.
    Uri resolved = previous.uri.resolveUri(uri);
    return _openUrl(method, resolved).then((MockHttpClientRequest request) {
      request
  ...
  void _returnConnection(MockHttpClientConnection connection) {
    _connectionTargets[connection.key]!.returnConnection(connection);
    _connectionsChanged();
  }

  // Remove a closed connection from the active set.
  void _connectionClosed(MockHttpClientConnection connection) {
    connection.stopTimer();
  ...
  void _connectionClosedNoFurtherClosing(MockHttpClientConnection connection) {
    connection.stopTimer();
    var connectionTarget = _connectionTargets[connection.key];
    if (connectionTarget != null) {
      connectionTarget.connectionClosed(connection);
  ...
  _ConnectionTarget _getConnectionTarget(String host, int port, bool isSecure) {
    String key = MockHttpClientConnection.makeKey(isSecure, host, port);
  ...
  Future<_ConnectionInfo> _getConnection(String uriHost, int uriPort, _ProxyConfiguration proxyConf, bool isSecure, MockHttpProfileData? profileData) {
  ...
  static String _findProxyFromEnvironment(Uri url, Map<String, String>? environment) {
    String? checkNoProxy(String? option) {
      ...
      while (names.moveNext()) {
        ...
        if ((name.startsWith("[") && name.endsWith("]") && "[${url.host}]" == name) || (name.isNotEmpty && url.host.endsWith(name))) {
          return "DIRECT";
        }
      }
      return null;
...
class MockHttpConnection extends LinkedListEntry<MockHttpConnection> with MockServiceObject {
  ...
  static Map<int, MockHttpConnection> _connections = new HashMap<int, MockHttpConnection>();
  ...
  MockHttpConnection(this._socket, this._httpServer) : _httpParser = new _HttpParser.requestParser() {
    ...
    _subscription = _httpParser.listen((incoming) {
      ...
      var outgoing = new MockHttpOutgoing(_socket);
      var response =
          new MockHttpResponse(incoming.uri!, incoming.headers.protocolVersion, outgoing, _httpServer.defaultResponseHeaders, _httpServer.serverHeader);
      // Parser found badRequest and sent out Response.
      if (incoming.statusCode == HttpStatus.badRequest) {
        response.statusCode = HttpStatus.badRequest;
      }
      var request = new MockHttpRequest(response, incoming, _httpServer, this);
      _streamFuture = outgoing.done.then((_) {
        response.deadline = null;
        if (_state == _DETACHED) return;
        if (response.persistentConnection && request.persistentConnection && incoming.fullBodyRead && !_httpParser.upgrade && !_httpServer.closed) {
          _state = _IDLE;
          _idleMark = false;
...
class MockHttpServer extends Stream<HttpRequest> with MockServiceObject implements HttpServer {
  // Use default Map so we keep order.
  static Map<int, MockHttpServer> _servers = new Map<int, MockHttpServer>();
  ...
  static Future<HttpServer> bind(address, int port, int backlog, bool v6Only, bool shared) {
    return ServerSocket.bind(address, port, backlog: backlog, v6Only: v6Only, shared: shared).then<HttpServer>((socket) {
      return new MockHttpServer._(socket, true);
    });
  }

  static Future<HttpServer> bindSecure(address, int port, SecurityContext? context, int backlog, bool v6Only, bool requestClientCertificate, bool shared) {
    return SecureServerSocket.bind(address, port, context, backlog: backlog, v6Only: v6Only, requestClientCertificate: requestClientCertificate, shared: shared)
        .then<HttpServer>((socket) {
      return new MockHttpServer._(socket, true);
    });
  }

  MockHttpServer._(this._serverSocket, this._closeServer) : _controller = new StreamController<HttpRequest>(sync: true) {
    _controller.onCancel = close;
    idleTimeout = const Duration(seconds: 120);
    _servers[_serviceId] = this;
  }

  MockHttpServer.listenOn(this._serverSocket)
      : _closeServer = false,
  ...
  static MockHttpHeaders _initDefaultResponseHeaders() {
    var defaultResponseHeaders = new MockHttpHeaders('1.1');
  ...
  StreamSubscription<HttpRequest> listen(void onData(HttpRequest event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    _serverSocket.listen((Socket socket) {
      if (socket.address.type != InternetAddressType.unix) {
        socket.setOption(SocketOption.tcpNoDelay, true);
      }
      // Accept the client connection.
      MockHttpConnection connection = new MockHttpConnection(socket, this);
  ...
  void _maybePerformCleanup() {
    ...
    if (closed && _idleConnections.isEmpty && _activeConnections.isEmpty && sessionManager != null) {
  ...
  void _handleRequest(MockHttpRequest request) {
    if (!closed) {
  ...
  void _connectionClosed(MockHttpConnection connection) {
    // Remove itself from either idle or active connections.
    connection.unlink();
    _maybePerformCleanup();
  }

  void _markIdle(MockHttpConnection connection) {
    _activeConnections.remove(connection);
    _idleConnections.add(connection);
  }

  void _markActive(MockHttpConnection connection) {
    _idleConnections.remove(connection);
    _activeConnections.add(connection);
  }

  // Lazy init.
  _HttpSessionManager get _sessionManager => _sessionManagerInstance ??= _HttpSessionManager();

  HttpConnectionsInfo connectionsInfo() {
    HttpConnectionsInfo result = new HttpConnectionsInfo();
    result.total = _activeConnections.length + _idleConnections.length;
    _activeConnections.forEach((MockHttpConnection conn) {
      if (conn._isActive) {
        result.active++;
      } else {
    ...
    _idleConnections.forEach((MockHttpConnection conn) {
      result.idle++;
      assert(conn._isIdle);
    });
    return result;
  ...
  final LinkedList<MockHttpConnection> _activeConnections = new LinkedList<MockHttpConnection>();
  final LinkedList<MockHttpConnection> _idleConnections = new LinkedList<MockHttpConnection>();
...
class _Proxy {
  ...
  const _Proxy(String this.host, int this.port, this.username, this.password) : isDirect = false;
...
class MockHttpConnectionInfo implements HttpConnectionInfo {
  ...
  MockHttpConnectionInfo(this.remoteAddress, this.remotePort, this.localPort);

  static MockHttpConnectionInfo? create(Socket socket) {
    if (socket == null) return null;
    try {
      return MockHttpConnectionInfo(socket.remoteAddress, socket.remotePort, socket.port);
    } catch (e) {}
    return null;
  }
}
...
class MockSocket extends Stream<Uint8List> implements Socket {
  ...
  MockSocket(this._incoming);

  StreamSubscription<Uint8List> listen(void onData(Uint8List event)?, {Function? onError, void onDone()?, bool? cancelOnError}) {
    return _incoming.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
  }

  Encoding get encoding => Utf8Codec();
...
abstract class _Credentials {
  MockHttpClientCredentials credentials;
  String realm;
  bool used = false;
  ...
  _Credentials(this.credentials, this.realm) {
    if (credentials.scheme == _AuthenticationScheme.DIGEST) {
      ...
      var creds = credentials as MockHttpClientDigestCredentials;
      var hasher = new _MD5()
...
class _SiteCredentials extends _Credentials {
  ...
  _SiteCredentials(this.uri, realm, MockHttpClientCredentials creds) : super(creds, realm);
  ...
  void authorize(HttpClientRequest request) {
    ...
    credentials.authorize(this, request as MockHttpClientRequest);
    used = true;
  }
}
...
class _ProxyCredentials extends _Credentials {
  ...
  _ProxyCredentials(this.host, this.port, realm, MockHttpClientCredentials creds) : super(creds, realm);
  ...
  void authorize(HttpClientRequest request) {
    ...
    credentials.authorizeProxy(this, request as MockHttpClientRequest);
  }
}

abstract class MockHttpClientCredentials implements HttpClientCredentials {
  _AuthenticationScheme get scheme;

  void authorize(_Credentials credentials, MockHttpClientRequest request);

  void authorizeProxy(_ProxyCredentials credentials, HttpClientRequest request);
}

class MockHttpClientBasicCredentials extends MockHttpClientCredentials implements HttpClientBasicCredentials {
  String username;
  String password;

  MockHttpClientBasicCredentials(this.username, this.password);
...
class MockHttpClientDigestCredentials extends MockHttpClientCredentials implements HttpClientDigestCredentials {
  String username;
  String password;

  MockHttpClientDigestCredentials(this.username, this.password);

  _AuthenticationScheme get scheme => _AuthenticationScheme.DIGEST;

  String authorization(_Credentials credentials, MockHttpClientRequest request) {
  ...
  void authorize(_Credentials credentials, HttpClientRequest request) {
    request.headers.set(HttpHeaders.authorizationHeader, authorization(credentials, request as MockHttpClientRequest));
  }

  void authorizeProxy(_ProxyCredentials credentials, HttpClientRequest request) {
    request.headers.set(HttpHeaders.proxyAuthorizationHeader, authorization(credentials, request as MockHttpClientRequest));
  }
}
